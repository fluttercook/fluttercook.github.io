---
title: On-device vision with Camera and ML Kit in Flutter — hướng dẫn Flutter
...
On-device vision keeps frames private and works offline. Bài này chuyển quan sát đó thành một mô hình nhỏ để bạn có thể kiểm thử, đo lường và giữ cho code dễ bảo trì khi app lớn lên.

## Vấn đề cốt lõi

Live barcode scan, face detect, and frame throttling. Câu hỏi hữu ích không phải API hay pattern có đẹp riêng lẻ hay không, mà là state nằm ở đâu, boundary nào chịu trách nhiệm khi lỗi xảy ra, và người dùng phục hồi thế nào khi happy path biến mất.

![Sơ đồ kiến trúc Flutter cho On-device vision with Camera and ML Kit in Flutter](/blog/images/fluttercook-editorial-2026.svg)
...
Đọc thêm các chủ đề liên quan: [Secure storage and biometrics for Flutter tokens](/vi/blog/flutter-secure-storage-biometrics/) và [Feature flags and remote config without a mess](/vi/blog/flutter-feature-flags-config/).
