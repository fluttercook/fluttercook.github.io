---
title: "Inside Kazumi: an open-source Flutter app worth studying"
package: "Kazumi"
repo: "Predidit/Kazumi"
githubUrl: "https://github.com/Predidit/Kazumi"
category: "App/Template"
stars: 27732
forks: 805
lastUpdate: "2026-07-16"
pubDev: ""
youtube: "https://www.youtube.com/results?search_query=flutter+kazumi"
priority: "High"
phase: "P1"
trendRank: 12
description: "基于自定义规则的番剧采集APP，支持流媒体在线观看，支持弹幕，支持实时超分辨率。."
seoDescription: "Kazumi: App/Template for Flutter with 27,732★ on GitHub. 基于自定义规则的番剧采集APP，支持流媒体在线观看，支持弹幕，支持实时超分辨率。. Install, usage, alternatives & FAQ."
keywords:
  - flutter Kazumi
  - Kazumi flutter
  - flutter app/template
  - flutter app
  - flutter template
  - flutter example app
  - flutter mobile app
  - Kazumi example
  - Kazumi tutorial
topics:
  - android
  - cross-platform
  - danmaku
  - flutter
  - ios
  - linux-desktop
related:
  - slug: immich
    title: 'Inside immich: an open-source Flutter app worth studying'
  - slug: localsend
    title: 'Inside localsend: an open-source Flutter app worth studying'
  - slug: open-source-ios-apps
    title: 'Inside open-source-ios-apps: an open-source Flutter app worth studying'
  - slug: spotube
    title: 'Inside spotube: an open-source Flutter app worth studying'
faq:
  - q: Is Kazumi free to use?
    a: Yes. Kazumi is open source and free to use in your Flutter projects. You can
      view the source, report issues, and contribute on GitHub.
  - q: Does Kazumi work on both iOS and Android?
    a: Kazumi is built for Flutter, so it targets iOS and Android from a single codebase,
      and typically web and desktop too depending on the project's platform support.
  - q: How popular is Kazumi?
    a: As of 2026, Kazumi has around 27,732 stars and 805 forks on GitHub, which puts
      it among the more widely used options in the App/Template space.
  - q: What are good alternatives to Kazumi?
    a: Popular alternatives in the App/Template category include immich, localsend,
      open-source-ios-apps. The best choice depends on your app's size, team, and performance
      needs.
datePublished: "2024-05-09"
dateModified: "2026-07-16"
draft: false
---

[`Kazumi`](https://github.com/Predidit/Kazumi) is an open-source **open-source app / starter template** for Flutter mobile app development, with **27,732★** on GitHub and last updated on **2026-07-16**. This guide covers what Kazumi does, why it matters in 2026, how to add it to your project, when to reach for it, and how it compares to the alternatives — plus a quick FAQ.

## What is Kazumi?

基于自定义规则的番剧采集APP，支持流媒体在线观看，支持弹幕，支持实时超分辨率。. It focuses on learning from a complete, real-world Flutter codebase. The project lives at [Predidit/Kazumi](https://github.com/Predidit/Kazumi) and is maintained by `Predidit`.

## Why Kazumi is worth knowing in 2026

Kazumi carries **27,732 GitHub stars**, **805 forks**, 446 open issues. It has been around since 2024, and is actively maintained (updated within the last month). For a App/Template option, that combination of adoption and upkeep usually means a healthy community, production usage, and plenty of examples to learn from — the things that make a dependency safe to build on.

## Running Kazumi

Kazumi is a complete project you can clone and run. Make sure you have the Flutter SDK installed, then:

```bash
git clone https://github.com/Predidit/Kazumi.git
cd Kazumi
flutter pub get
flutter run
```

From there, read through the project structure to see how a real Flutter app is organised — routing, state, data access, and UI — and fork it as a starting point for your own build.

## When should you use Kazumi?

Reach for Kazumi when:

- you want a production-grade example to study or fork
- you're bootstrapping a new app from a proven starter
- you learn best by reading complete projects

It's especially relevant if your project touches `android`, `cross-platform`, `danmaku`, `flutter`, `ios`, `linux-desktop`.

## Kazumi vs. the alternatives

If you're weighing options in the **App/Template** space, these are the other projects developers most often compare Kazumi against:

- [Inside immich: an open-source Flutter app worth studying](/recipes/immich/)
- [Inside localsend: an open-source Flutter app worth studying](/recipes/localsend/)
- [Inside open-source-ios-apps: an open-source Flutter app worth studying](/recipes/open-source-ios-apps/)
- [Inside spotube: an open-source Flutter app worth studying](/recipes/spotube/)

There's no single winner — the right pick depends on your app's size, your team's familiarity, and your performance budget. Browse the full [App/Template collection](/recipes/) to compare them side by side.

## Frequently asked questions

### Is Kazumi free to use?

Yes. Kazumi is open source and free to use in your Flutter projects. You can view the source, report issues, and contribute on GitHub.

### Does Kazumi work on both iOS and Android?

Kazumi is built for Flutter, so it targets iOS and Android from a single codebase, and typically web and desktop too depending on the project's platform support.

### How popular is Kazumi?

As of 2026, Kazumi has around 27,732 stars and 805 forks on GitHub, which puts it among the more widely used options in the App/Template space.

### What are good alternatives to Kazumi?

Popular alternatives in the App/Template category include immich, localsend, open-source-ios-apps. The best choice depends on your app's size, team, and performance needs.

## Resources & links

- **GitHub:** [Predidit/Kazumi](https://github.com/Predidit/Kazumi)
- **Video tutorials:** [search YouTube for Kazumi](https://www.youtube.com/results?search_query=flutter+kazumi)

---

*Part of [FlutterCook](/recipes/) — 500 hands-on guides to the best open-source Flutter libraries, UI kits, and apps. Explore the live [GitHub trends](/trends/) or browse [YouTube guides](/youtube/).*
