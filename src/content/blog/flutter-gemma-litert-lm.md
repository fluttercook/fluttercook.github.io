...
On-device models keep data local, work offline, and cut per-request cost. This article turns that observation into a small implementation model you can test, measure, and keep boring when the app grows.

## The problem in one sentence

Local inference across six Flutter platforms with GPU/NPU acceleration. The useful question is not whether the API or pattern looks elegant in isolation. It is where state lives, which boundary owns failure, and how a user recovers when the happy path disappears.

![Flutter architecture diagram for On-device Gemma with flutter_gemma and LiteRT-LM](/blog/images/fluttercook-editorial-2026.svg)
...
For the neighboring ideas, read [Dot shorthands in Flutter: cleaner enum and const syntax](/blog/flutter-dot-shorthands/) and [Flutter Property Editor: inspect widgets without print debugging](/blog/flutter-property-editor-devtools/).
