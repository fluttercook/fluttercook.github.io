---
title: 'Patrol: integration tests that can leave the Flutter sandbox'
description: 'Showcase patrol in a production-minded Flutter architecture: native
  permissions, notifications, deep links, and device automation.'
seoDescription: 'Showcase patrol in a production-minded Flutter architecture: native
  permissions, notifications, deep links, and device automation.'
keywords:
- patrol
- Flutter open source
- Flutter library
- Flutter tutorial
category: Open Source
topic: Flutter OSS
level: Intermediate
author: Trung Hieu
publishDate: '2026-09-16'
emoji: 🧩
tags:
- Flutter
- OpenSource
- Testing
sources:
- name: Patrol on pub.dev
  url: https://pub.dev/packages/patrol
- name: Patrol repository
  url: https://github.com/leancodepl/patrol
related:
- slug: flutter-state-management-decision-guide
  title: State management decision guide
- slug: flutter-testing-widget-golden-integration
  title: Widget and golden testing
draft: false
---

# Patrol: integration tests that can leave the Flutter sandbox

`patrol` is useful because it focuses on native permissions, notifications, deep links, and device automation. The important engineering move is to place the package behind a clear boundary, so your Flutter UI depends on a stable capability rather than a vendor-shaped API.

![FlutterCook open-source showcase](/blog/images/fluttercook-editorial-2026.svg)

## What the library should own

- Keep package calls inside a named adapter or feature boundary.
- Make lifecycle, errors, and loading state part of a testable contract.
- Expose only the capability the app needs; hide implementation details from the whole tree.

## A focused starting point

```dart
patrolTest('camera permission flow', ($) async {
  await $.native.grantPermissionWhenInUse();
  await $.pumpWidgetAndSettle(const CameraApp());
  await $(#takePhoto).tap();
  expect(find.byType(PhotoPreview), findsOneWidget);
});
```

## Production checklist

1. Read the README and changelog for the exact version you pin.
2. Add one test for lifecycle, failure, and app background/foreground behavior.
3. Verify every platform your product supports, not only the developer machine.
4. Record ownership, upgrade cadence, and rollback notes in the repository.

## Common pitfall

Keep native setup deterministic; a test that depends on a previously granted permission is flaky.

## Takeaway

A good open-source library does not replace architecture. It makes one difficult boundary clearer, observable, and easier to replace.
