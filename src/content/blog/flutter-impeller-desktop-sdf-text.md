...
Desktop Impeller defaults land with Signed Distance Function text and Wide Gamut on macOS. This article turns that observation into a small implementation model you can test, measure, and keep boring when the app grows.

## The problem in one sentence

Flutter 3.47 makes Impeller the default desktop renderer — what changes for text and jank. The useful question is not whether the API or pattern looks elegant in isolation. It is where state lives, which boundary owns failure, and how a user recovers when the happy path disappears.

![Flutter architecture diagram for Impeller on desktop: SDF text and Metal/Vulkan defaults](/blog/images/fluttercook-editorial-2026.svg)
...
For the neighboring ideas, read [Swift Package Manager plugin migration checklist for Flutter](/blog/flutter-swiftpm-plugin-migration-checklist/) and [Hybrid Composition++: Vulkan-composited platform views](/blog/flutter-hcpp-platform-views-vulkan/).
