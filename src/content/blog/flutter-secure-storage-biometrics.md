...
Refresh tokens in SharedPreferences are a security incident waiting to happen. This article turns that observation into a small implementation model you can test, measure, and keep boring when the app grows.

## The problem in one sentence

Keychain/Keystore, biometric gates, and token rotation. The useful question is not whether the API or pattern looks elegant in isolation. It is where state lives, which boundary owns failure, and how a user recovers when the happy path disappears.

![Flutter architecture diagram for Secure storage and biometrics for Flutter tokens](/blog/images/fluttercook-editorial-2026.svg)
...
For the neighboring ideas, read [Feature flags and remote config without a mess](/blog/flutter-feature-flags-config/) and [Codegen that pays for itself: freezed and json_serializable](/blog/flutter-codegen-freezed-json/).
