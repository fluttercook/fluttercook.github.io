...
description: Immutable models, unions, and less hand-written parse code.
seoDescription: Flutter freezed json_serializable codegen, immutable models unions build_runner. Practical guide for production
  Flutter teams.
keywords:
- freezed
- json_serializable
- build_runner flutter
- immutable models dart
- dart codegen
- flutter
- dart
- mobile development
...
Hand-written JSON parsers rot. Codegen keeps models honest with the API. This article turns that observation into a small implementation model you can test, measure, and keep boring when the app grows.

## The problem in one sentence

Immutable models, unions, and less hand-written parse code. The useful question is not whether the API or pattern looks elegant in isolation. It is where state lives, which boundary owns failure, and how a user recovers when the happy path disappears.

![Flutter architecture diagram for Codegen that pays for itself: freezed and json_serializable](/blog/images/fluttercook-editorial-2026.svg)
...
For the neighboring ideas, read [Melos monorepos for Flutter apps and packages](/blog/flutter-melos-monorepo/) and [GitHub Actions for Flutter with sane caches](/blog/flutter-ci-github-actions-cache/).
