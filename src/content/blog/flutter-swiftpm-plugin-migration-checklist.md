...
CocoaPods is maintenance mode. Flutter 3.44+ resolves iOS/macOS deps with SwiftPM by default. This article turns that observation into a small implementation model you can test, measure, and keep boring when the app grows.

## The problem in one sentence

What iOS/macOS plugin authors must ship after SwiftPM became the default. The useful question is not whether the API or pattern looks elegant in isolation. It is where state lives, which boundary owns failure, and how a user recovers when the happy path disappears.

![Flutter architecture diagram for Swift Package Manager plugin migration checklist for Flutter](/blog/images/fluttercook-editorial-2026.svg)
...
For the neighboring ideas, read [Hybrid Composition++: Vulkan-composited platform views](/blog/flutter-hcpp-platform-views-vulkan/) and [Agentic Hot Reload: closing the prompt-to-pixels loop](/blog/flutter-agentic-hot-reload-workflow/).
