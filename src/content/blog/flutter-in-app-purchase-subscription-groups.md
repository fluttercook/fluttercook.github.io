...
description: How to structure tiers so users do not double-pay.
seoDescription: Subscription groups proration upgrade downgrade mobile IAP. Practical guide for production Flutter teams.
keywords:
- subscription groups
- proration
- upgrade downgrade
...
related:
- slug: flutter-freemium-paywall-ux
...
Confusing tiers cause refunds and support load. This article turns that observation into a small implementation model you can test, measure, and keep boring when the app grows.

## The problem in one sentence

How to structure tiers so users do not double-pay. The useful question is not whether the API or pattern looks elegant in isolation. It is where state lives, which boundary owns failure, and how a user recovers when the happy path disappears.

![Flutter architecture diagram for Subscription groups, upgrades, and proration explained](/blog/images/fluttercook-editorial-2026.svg)
...
For the neighboring ideas, read [Paywall UX that converts without dark patterns](/blog/flutter-freemium-paywall-ux/) and [A mobile analytics event schema that survives redesigns](/blog/flutter-analytics-events-schema/).
