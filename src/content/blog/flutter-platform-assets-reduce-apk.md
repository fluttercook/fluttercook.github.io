...
description: 'pubspec platforms: keys keep desktop files out of mobile APKs.'
seoDescription: Flutter platform-specific assets pubspec platforms, reduce APK size, exclude desktop assets mobile. Practical
  guide for production Flutter teams.
keywords:
- flutter platform assets
- pubspec platforms key
...
One assets list used to mean every platform received every file. 3.41 filters by platform. This article turns that observation into a small implementation model you can test, measure, and keep boring when the app grows.

## The problem in one sentence

pubspec platforms: keys keep desktop files out of mobile APKs. The useful question is not whether the API or pattern looks elegant in isolation. It is where state lives, which boundary owns failure, and how a user recovers when the happy path disappears.

![Flutter architecture diagram for Ship only the assets each platform needs](/blog/images/fluttercook-editorial-2026.svg)
...
For the neighboring ideas, read [UIScene lifecycle: the iOS 27 launch gate for Flutter apps](/blog/flutter-uiscene-ios-27-checklist/) and [Fragment shaders: bind uniforms by name in Flutter](/blog/flutter-fragment-shader-uniforms-by-name/).
