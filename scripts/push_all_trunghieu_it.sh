...
for p in sorted(Path('src/content/blog').glob('*.md')):
    text = p.read_text()
    if 'publishDate: "2026-09-10"' in text or "publishDate: '2026-09-11'" in text:
        print(p.stem)
PY
)
echo "== EN posts =="
python3 scripts/publish_to_blogger.py --blog "$BLOG" --collection blog --lang en "${ARGS[@]}" --publish --skip-existing --limit "$LIMIT" --sleep 4
echo "== VI posts =="
python3 scripts/publish_to_blogger.py --blog "$BLOG" --collection blog --lang vi "${ARGS[@]}" --publish --skip-existing --limit "$LIMIT" --sleep 4
